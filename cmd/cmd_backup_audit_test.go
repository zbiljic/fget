...
func TestWriteBackupAuditFileUsesCanonicalTemporaryName(t *testing.T) {
	directory := t.TempDir()
	path := filepath.Join(directory, "manifest.json")
	if err := writeBackupAuditFile(path, func(w io.Writer) error {
		entries, err := os.ReadDir(directory)
		if err != nil {
			return err
		}
		if len(entries) != 1 || !entries[0].Type().IsRegular() ||
			!strings.HasPrefix(entries[0].Name(), ".fget-backup-audit-") ||
			!strings.HasSuffix(entries[0].Name(), ".tmp") {
			return fmt.Errorf("temporary entries = %v", entries)
		}
