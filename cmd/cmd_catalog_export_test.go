...
func TestWriteCatalogExportFileUsesCanonicalTemporaryName(t *testing.T) {
	t.Parallel()

	directory := t.TempDir()
	path := filepath.Join(directory, "inventory.jsonl")
	if err := writeCatalogExportFile(path, func(w io.Writer) error {
		...
		if len(entries) != 1 || !entries[0].Type().IsRegular() ||
			!strings.HasPrefix(entries[0].Name(), ".fget-catalog-export-") ||
