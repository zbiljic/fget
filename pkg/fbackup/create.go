...
func Create(ctx context.Context, options CreateOptions) error {
	...
	if err := writeJSONAtomic(metadataPath, metadata, 0o644, ".fget-backup-metadata-*.tmp"); err != nil {
		return err
	}
	...
	return writeJSONAtomic(metadataPath, metadata, 0o644, ".fget-backup-metadata-*.tmp")
}
...
func openOrInitialize(destination string, manifest Manifest, auditHash string) (BackupMetadata, error) {
	...
	for _, entry := range entries {
		if entry.Type().IsRegular() && isStaleBackupTemp(entry.Name()) {
...
func createRepository(
	...
	case ClassificationDelta:
		specifications = append(specifications,
			...
			struct{ kind, name string }{"index-patch", "staged.patch"},
			struct{ kind, name string }{"patch", "unstaged.patch"},
	...
	for _, specification := range specifications {
		...
		temporary, err := os.CreateTemp(filepath.Dir(destinationPath), ".fget-backup-artifact-*.tmp")
		if err != nil {
			return err
		}
		...
		if err := writeJSONAtomic(filepath.Join(destination, "backup.json"), metadata, 0o644, ".fget-backup-metadata-*.tmp"); err != nil {
			return err
		}
		if progress != nil {
...
func isStaleBackupTemp(name string) bool {
	if !strings.HasSuffix(name, ".tmp") {
		return false
	}
	for _, prefix := range []string{
		".fget-backup-artifact-",
		".fget-backup-metadata-",
	} {
