...
func TestCreateVerifyDeltaAndResume(t *testing.T) { //nolint:gocyclo // integration coverage intentionally exercises all delta artifacts
	...
	wantKinds := map[string]bool{"bundle": true, "index-patch": true, "patch": true, "untracked": true}
	wantNames := map[string]string{"bundle": "repository.bundle", "index-patch": "staged.patch", "patch": "unstaged.patch", "untracked": "untracked.tar.gz"}
	for _, artifact := range metadata.Artifacts {
		if !wantKinds[artifact.Kind] {
			t.Fatalf("unexpected artifact kind %q", artifact.Kind)
		}
		if filepath.Base(artifact.Path) != wantNames[artifact.Kind] {
			t.Fatalf("artifact %q path = %q, want %q", artifact.Kind, artifact.Path, wantNames[artifact.Kind])
		}
		delete(wantKinds, artifact.Kind)
	}
	if len(wantKinds) != 0 {
		t.Fatalf("missing artifact kinds: %v", wantKinds)
	}
	...
	for _, name := range []string{"repository.bundle", "unstaged.patch", "untracked.tar.gz"} {
	...
	patch, err := os.ReadFile(filepath.Join(destination, "repos", repositoryDirName("example/repo"), "unstaged.patch"))
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Contains(patch, []byte("GIT binary patch")) {
		t.Fatal("unstaged.patch does not contain the binary worktree layer")
	}
...
func TestDeltaSeparatesIndexAndWorktreeLayers(t *testing.T) {
	...
	indexPatch, err := os.ReadFile(filepath.Join(base, "staged.patch"))
	if err != nil {
		t.Fatal(err)
	}
	trackedPatch, err := os.ReadFile(filepath.Join(base, "unstaged.patch"))
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Contains(indexPatch, []byte("+B")) || !bytes.Contains(trackedPatch, []byte("-B")) || !bytes.Contains(trackedPatch, []byte("+A")) {
		t.Fatalf("staged.patch/unstaged.patch did not preserve layers: staged=%q unstaged=%q", indexPatch, trackedPatch)
	}
	if err := Verify(context.Background(), destination, true); err != nil {
		t.Fatal(err)
	}
